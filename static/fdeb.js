...
Code from: http://bl.ocks.org/upphiminn/6515478
  ...
  d3.ForceEdgeBundling = function(){
    var data_nodes = {},        // {'nodeid':{'x':,'y':},..}
        ...
        function edge_as_vector(P){
            return {'x': data_nodes[P.target.index].x - data_nodes[P.source.index].x,
                    'y': data_nodes[P.target.index].y - data_nodes[P.source.index].y}
        }

        function edge_length(e){
            return Math.sqrt(Math.pow(data_nodes[e.source.index].x-data_nodes[e.target.index].x, 2) +
                             Math.pow(data_nodes[e.source.index].y-data_nodes[e.target.index].y, 2));
        }
        ...
        function edge_midpoint(e){
            var middle_x = (data_nodes[e.source.index].x + data_nodes[e.target.index].x) / 2.0;
            var middle_y = (data_nodes[e.source.index].y + data_nodes[e.target.index].y) / 2.0;
        ...
        function initialize_edge_subdivisions()
        {
            for(var i = 0; i < data_edges.length; i++)
             if(P_initial == 1)
                ...
                subdivision_points_for_edge[i].push(data_nodes[data_edges[i].source.index]);
                subdivision_points_for_edge[i].push(data_nodes[data_edges[i].target.index]);
            }
        }
        ...
        function filter_self_loops(edgelist){
            ...
            for(var e=0; e < edgelist.length; e++){
                if(data_nodes[edgelist[e].source.index].x != data_nodes[edgelist[e].target.index].x  &&
                   data_nodes[edgelist[e].source.index].y != data_nodes[edgelist[e].target.index].y ){ 
                    //or smaller than eps
        ...
        function update_edge_divisions(P){
            for(var e_idx=0; e_idx < data_edges.length; e_idx++){
                ...
                if( P == 1 ){
                    subdivision_points_for_edge[e_idx].push(data_nodes[data_edges[e_idx].source.index]); // source
                    subdivision_points_for_edge[e_idx].push(edge_midpoint(data_edges[e_idx])); // mid point
                    subdivision_points_for_edge[e_idx].push(data_nodes[data_edges[e_idx].target.index]); // target
                }else{
                    ...
                    new_subdivision_points.push(data_nodes[data_edges[e_idx].source.index]); //source
                    ...
                    new_subdivision_points.push(data_nodes[data_edges[e_idx].target.index]); //target
        ...
        function position_compatibility(P, Q){
            var lavg = (edge_length(P) + edge_length(Q))/2.0;
            var midP = {'x':(data_nodes[P.source.index].x + data_nodes[P.target.index].x)/2.0,
                        'y':(data_nodes[P.source.index].y + data_nodes[P.target.index].y)/2.0};
            var midQ = {'x':(data_nodes[Q.source.index].x + data_nodes[Q.target.index].x)/2.0,
                        'y':(data_nodes[Q.source.index].y + data_nodes[Q.target.index].y)/2.0};
            var result = lavg/(lavg + euclidean_distance(midP, midQ));
            return result;
        }

        function edge_visibility(P, Q){
            var I0 = project_point_on_line(data_nodes[Q.source.index], {'source':data_nodes[P.source.index],
                                                                  'target':data_nodes[P.target.index]});
            var I1 = project_point_on_line(data_nodes[Q.target.index], {'source':data_nodes[P.source.index], 
                                                                  'target':data_nodes[P.target.index]}); //send acutal edge points positions
